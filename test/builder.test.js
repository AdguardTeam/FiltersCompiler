...
import { build, include } from '../src/main/builder';
...
describe('Test builder', async () => {
    ...
    describe('Works', () => {
        ...
        describe('condition directives for platforms', () => {
            ...
            it('filters4.txt - adguard_ext_edge_mv3 constant for the if directive', async () => {
                const ifContent = await readFile(path.join(platformsDir, 'edge-mv3', 'filters', '4.txt'));
                expect(ifContent).toBeTruthy();

                const ifLines = ifContent.split(/\r?\n/);
                expect(ifLines.length).toEqual(24);
                expect(ifLines.includes('edge_mv3_specific_rule')).toBeTruthy();
                expect(ifLines.includes('ios_rule')).toBeFalsy();
            });
